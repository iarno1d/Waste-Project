...
class _ReportState extends State<Report> {
  ...
  Future<void> _handleImage(ImageSource source) async {
    ...
    try {
      // ⚡ Step 1: classify only — fast
      final result = await _ticketService.classifyOnly(picked);
      if (!mounted) return;
      ...
      // 💾 Step 2: save silently in background
      _ticketService.saveTicketInBackground(picked, result);
    } catch (e) {
      if (!mounted) return;
      setState(() => _isClassifying = false);
